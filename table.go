...
// Render table output
func (t *Table) Render(highLight int) {
	if t.borders.Top {
		t.printLine(true)
	}
	t.printHeading()
...
//  @Description:
//  @receiver t
//  @param highLight 高亮行数，如果为负数，表示不高亮，从0开始，表示第一行高亮
//
func (t Table) printRows(highLight int) {
	for i, lines := range t.lines {
		if i == highLight  {
			t.printRow(lines, i,true)
		}else {
			t.printRow(lines, i,false)
		}
...
func (t *Table) printRow(columns [][]string, rowIdx int,isHighLight bool) {
	// Get Maximum Height
	max := t.rs[rowIdx]
	total := len(columns)
